...
class LoadConfig(BaseConfig[LoadStrategy]):
    ...
    body: dict | None = Field(default=None, description="Request body for POST/PUT requests.")

    duration_seconds: int = Field(
        default=60, gt=0, description="Duration of the load test in seconds."
    )
    interval_seconds: float = Field(
        default=1.0, gt=0, description="Interval between requests in seconds. (For linear load strategy)"
    )
